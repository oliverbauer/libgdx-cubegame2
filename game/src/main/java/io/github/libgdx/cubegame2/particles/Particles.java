package io.github.libgdx.cubegame2.particles;
...
public class Particles {
	...
	public Particles(PerspectiveCamera cam, float x,float y, float z, Color c) {
		...
		addEmitter(c, particleTexture, tmpVector.set(x, y, z));
	}
	...
	private ParticleController addEmitter(Color c, Texture particleTexture, Vector3 translation) {
		ParticleController controller = createBillboardController(c, particleTexture);
		controller.init();
		controller.start();
	...
	private ParticleController createBillboardController(Color c, Texture particleTexture) {
		// Emission
		...
		scaleInfluencer.value.setTimeline(new float[] { 0, 1f }); // last value: how far particles move
		...
		colorInfluencer.colorValue.setColors(new float[] { c.r, c.g, c.b, 0, 0, 0 });
